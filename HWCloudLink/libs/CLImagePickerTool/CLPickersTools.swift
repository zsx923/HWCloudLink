...
//    还有一个与 iCloud 密切相关的属性?progressHandler，当图像需要从 iCloud 下载时，这个 block 会被自动调用，block 中会返回图像下载的进度，图像的信息，出错信息。开发者可以利用这些信息反馈给用户当前图像的下载进度以及状况，但需要注意?progressHandler 不在主线程上执行，因此在其中需要操作 UI，则需要手工放到主线程执行
    ...
    // 用户是否开启权限
    func authorizeWithSuperVC(superVC:UIViewController?, authorizeClouse:@escaping (PHAuthorizationStatus)->()){
        self.superVC = superVC
        self.authorize(authorizeClouse: authorizeClouse)
    }
    ...
    func authorize(authorizeClouse:@escaping (PHAuthorizationStatus)->()){
        ...
        if status == .authorized{
            if let parentVC = self.superVC {
                if dataArr.first?.count ?? 0 <= 0 {
                    // 无照片时，先弹出选择部分照片弹框
                    if #available(iOS 14, *) {
                        PHPhotoLibrary.shared().presentLimitedLibraryPicker(from: parentVC)
                    } else {
