...
// 所有照片的可能情况
let allPhoto = tr("所有照片")
let allPhoto2 = "All Photos"
let allPhoto3 = "Camera Roll"
let allPhoto4 = tr("相机胶卷")
let allPhoto5 = tr("最近项目")
...
//let maxPhotoCountStr = BundleUtil.cl_localizedStringForKey(key: "最多选择3张图片")
...
let sureStr = tr("确定")
let resetStr = tr("重置")
let previewStr = tr("预览")
let cancelStr = tr("取消")
let photoStr = tr("相册")
let errorStr = tr("错误")
let editorStr = tr("编辑")
let playErrorStr = tr("播放出错")
let maxPhotoCountStr = tr("最多选择3张图片")
let knowStr = tr("知道了")
let imageAndVideoOnlyOneStr = tr("视频文件和图片文件只能选择1种")

let graffitiStr = tr("涂鸦")
let eraserStr = tr("橡皮擦")
let mosaicStr = tr("马赛克")
let undoStr = tr("上一步")
let redoStr = tr("下一步")

let photoLimitStr = tr("照片访问受限")
let clickSetStr = tr("点击“设置”，允许访问您的照片")
let setStr = tr("设置")

let cameraLimitStr = tr("相机访问受限")
let clickCameraStr = tr("点击“设置”，允许访问您的相机")

let tackPhotoStr = tr("拍照")
let chooseStr = tr("从手机相册选择")

let favStr = tr("收藏")
let videoStr = tr("视频")
let allPStr = tr("所有照片")
let rencentStr = tr("最近添加")
let shotStr = tr("屏幕快照")
let selfStr = tr("自拍")
let delectStr = tr("最近删除")
