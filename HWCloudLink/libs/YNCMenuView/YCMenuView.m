...
+ (instancetype)actionWithTitle:(NSString *)title titleColor:(UIColor *)color image:(UIImage *)image  handler:(void (^)(YCMenuAction *action))handler {
    YCMenuAction *action = [[YCMenuAction alloc] initWithTitle:title titleColor:color image:image handler:handler];
    return action;
}
- (instancetype)initWithTitle:(NSString *)title titleColor:(UIColor *)color image:(UIImage *)image handler:(void (^)(YCMenuAction *))handler{
    if (self = [super init]) {
        _title = title;
        _image = image;
        _textColor = color;
...
- (void)dismiss{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(yCMenuViewDismiss)]) {
        [_delegate yCMenuViewDismiss];
    }
...
//    cell.textLabel.textColor = _textColor;
    if (action.textColor != nil) {
        ...
        cell.textLabel.textColor = [UIColor colorWithLightColor:_textColor DarkColor:[UIColor whiteColor]];
    }
...
//        _contentView.backgroundColor = _menuColor;
        _contentView.backgroundColor = [UIColor colorWithLightColor:_menuColor DarkColor:[UIColor colorWithHexString:@"#262626"]];
