...
//        } else {
    ...
    func gotoPhoto(MaxImagesCount: Int) {
        // 判断用户是否开启访问相册功能
        CLPickersTools.instence.authorizeWithSuperVC(superVC: self.superVC, authorizeClouse: { (state) in
            if state == .authorized {
        ...
        CLPickersTools.instence.authorizeCamaro { (state) in
            if state == .authorized {
                if self.isCameraAvailable() == false {
                    PopViewUtil.alert(message: tr("相机不可用"), leftTitle: "", rightTitle: tr("确定"), leftHandler: nil, rightHandler: nil)
                    return
                }
