    ...
    @objc func setupCellCover() {
        ...
        if self.onlyChooseImageOrVideo {
            if self.chooseImageBtn.isSelected {
                ...
                self.backgroundColor = UIColor.colorWithSystem(lightColor: UIColor.white, darkColor: UIColor.black)
                self.chooseImageBtn.isHidden = false
            } else {
                self.iconView.alpha = (self.model?.onlyChooseImageOrVideo ?? false) ? 0.5:1
                self.backgroundColor = UIColor.colorWithSystem(lightColor: UIColor(white: 0, alpha: 0.8), darkColor: UIColor(white: 1, alpha: 0.2))
