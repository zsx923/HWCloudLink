//
//  HWAppInfoManager.swift
//  HWCloudLink
//
//  Created by yuepeng on 2021/1/27.
//  Copyright © 2021 陈帆. All rights reserved.
//


#warning("获取设备信息用于开发阶段的日志收集，APPstore上线时可能会影响其审核政策通过，请注意！！！！")
            ...
            if let carriers = info.serviceSubscriberCellularProviders {
                if carriers.keys.count == 0 {
                    ...
                    for (index, carrier) in carriers.values.enumerated() {
                        guard carrier.carrierName != nil else { return tr("无手机卡") }
...
class HWAppInfoManager: NSObject {
    

    func getTotalDeviceInfo() -> Void {
        guard let infoDictionary = Bundle.main.infoDictionary else { return }
        if let appDisplayName = infoDictionary["CFBundleName"] as? String {
            CLLog("appName:\(NSString.encryptNumber(with: appDisplayName) ?? "")")
        }
        if let majorVersion = infoDictionary["CFBundleShortVersionString"] as? String,
           let minorVersion = infoDictionary["CFBundleVersion"] as? String {
            CLLog("<appVersion:\(majorVersion)(build:\(minorVersion))>")
        }
        
        let deviceModel = DeviceInfo.zz_getDeviceName()
        let sysVersion = UIDevice.current.systemVersion
        CLLog("<deviceModel:\(deviceModel)>")
        CLLog("<sysVersion:\(sysVersion)>")
        CLLog("<cpuType:(\(DeviceInfo.zz_getDeviceCpuType()),cpuCount:(\(DeviceInfo.zz_getDeviceCpuCount()))>")
        CLLog("ip:\(NSString.encryptIP(with: DeviceInfo.zz_getDeviceIP()) ?? "")")
        CLLog("<diskTotalSize:\(DeviceInfo.zz_getDeviceDiskTotalSize()),available:\(DeviceInfo.zz_getDeviceAvalibleDiskSize())>")
        CLLog("<deviceSupplier:\(String(describing: DeviceInfo.zz_getDeviceSupplier()))>")
        CLLog("<systemVersion:\( DeviceInfo.zz_getDeviceSystemVersion())>")
