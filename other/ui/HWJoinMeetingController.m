...
@interface HWJoinMeetingController () <UITextFieldDelegate>
...
@implementation HWJoinMeetingController
...
- (void)setupViews
{
    ...
    [closeBtn setBackgroundImage:[UIImage imageNamed:@"fm_btn_close"] forState:UIControlStateNormal];
    ...
    self.meetingIdTextField.hidden = NO;
    
    NSString *str = @"如有密码，请输入";
    ...
    [showPwdBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.meetingIdTextField.mas_bottom).offset(38);
...
- (UITextField *)meetingIdTextField
{
    if (!_meetingIdTextField)
    {
        ...
        NSAttributedString *string = [[NSAttributedString alloc] initWithString:@"请输入会议号" attributes:dict];
                
        _meetingIdTextField = [[UITextField alloc] init];
        _meetingIdTextField.backgroundColor = [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:0.1];
        _meetingIdTextField.attributedPlaceholder = string;
        _meetingIdTextField.textAlignment = NSTextAlignmentCenter;
        _meetingIdTextField.font = [UIFont fontWithName:@"PingFangSC-Regular" size:20.0f];
        _meetingIdTextField.textColor = [UIColor whiteColor];
        _meetingIdTextField.delegate = self;
        _meetingIdTextField.layer.cornerRadius = 6;
        [self.view addSubview:_meetingIdTextField];

        [_meetingIdTextField mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (UITextField *)passwordTextField
{
    if (!_passwordTextField)
    {
        ...
        [_passwordTextField mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self.meetingIdTextField.mas_left).offset(0);
            make.top.mas_equalTo(self.meetingIdTextField.mas_bottom).offset(30);
            make.right.mas_equalTo(self.meetingIdTextField.mas_right).offset(0);
            make.height.mas_equalTo(self.meetingIdTextField.mas_height);
        }];
    }
...
- (UIButton *)microBtn
{
    if (!_microBtn)
    {
        ...
        [_microBtn setBackgroundImage:[UIImage imageNamed:@"btn_normal"] forState:UIControlStateNormal];
...
- (UIButton *)cameraBtn
{
    if (!_cameraBtn)
    {
        ...
        [_cameraBtn setBackgroundImage:[UIImage imageNamed:@"btn_meeting_click"] forState:UIControlStateNormal];
