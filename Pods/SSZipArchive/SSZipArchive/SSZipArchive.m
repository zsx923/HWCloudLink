...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(nullable NSString *)fileName compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    if (!fileName) {
        if ([path containsString:@"TUPC60log"]) {
          path = [self distinguishTsdkPaths:path rangeName:@"TUPC60log/tsdk"];
        }
        if ([path containsString:@"/Log"]) {
            path = [self distinguishTsdkPaths:path rangeName:@"Documents"];
        }
        if ([path containsString:@"/Caches"]) {
            path = [self distinguishTsdkPaths:path rangeName:@"Caches"];
        }
        fileName = path;
    }
...
- (NSString *)distinguishTsdkPaths:(NSString *)sdkPaths rangeName :(NSString*)rangeName{
    if (sdkPaths.length == 0) {
        return sdkPaths;
    }
    NSRange range = [sdkPaths rangeOfString:rangeName];
    NSString *tmpStr = [sdkPaths substringFromIndex:range.location +range.length];
    return tmpStr;
}
